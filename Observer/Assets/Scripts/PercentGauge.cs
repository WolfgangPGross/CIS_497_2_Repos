...
public class PercentGauge : MonoBehaviour
{
    ...
    public void DealKnockback(float knockback, Vector3 attackDirection , Transform attackOrigin, int toWhichPlayer)
    {
        ...
        if (toWhichPlayer == 1) //If player_01
        {
            ...
            //rotate the player towards the attack origin
            //Debug.Log("Looking towards attack Origin: " + attackDirection.position);
            //player_01_rigidbody.transform.LookAt(attackOrigin.position);
            
            //player_01_rigidbody.AddForce(transform.TransformDirection(0,0,1) * knockback, ForceMode.Impulse);
            player_01_rigidbody.AddForce(attackDirection * knockback, ForceMode.Impulse);
        }
        else if (toWhichPlayer == 2) //If player_02
        {
            //Handle knockback here
            ...
            //rotate the player towards the attack origin
            //Debug.Log("Looking towards attack Origin: " + attackOrigin.position);
            //player_02_rigidbody.transform.LookAt(attackOrigin.position);

            //player_02_rigidbody.AddForce(transform.TransformDirection(0, 0, 1) * knockback, ForceMode.Impulse);
            player_02_rigidbody.AddForce(attackDirection * knockback, ForceMode.Impulse);
        }
    }
